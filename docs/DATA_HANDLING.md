...
The source tree contains application logic, report definitions, adapters, and municipal branding. It does not contain the Municipality's raw CBMS JSON data, `.RData`/`.RDS` files, PSA RSA keys, or generated release binaries.
...
Never upload municipal raw data to GitHub, even to a private repository unless the LGU's authorization explicitly allows it. Do not put data into issues, pull requests, workflow logs, screenshots, or test fixtures.
